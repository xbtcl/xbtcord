...
// Only import pure modules that don't import other parts of Xbtcord.
...
const setCssDebounced = debounce((css: string) => XbtcordNative.quickCss.set(css));

const themeStore = DataStore.createStore("XbtcordThemes", "XbtcordThemeData");

// probably should make this less cursed at some point
window.XbtcordNative = {
    themes: {
    ...
    quickCss: {
        get: () => DataStore.get("XbtcordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("XbtcordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "XbtcordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => XbtcordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("XbtcordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("XbtcordSettings", JSON.stringify(s)),
